...
> [!NOTE]
> v1.1.0では、必要なPythonパッケージをTracker専用の仮想環境（venv）へインストールします。ユーザーのPython環境とは分離されるため、他のPythonアプリやスクリプトへ影響しません。v1.0.1でユーザー環境へインストール済みのパッケージは安全のため自動削除せず、Python本体もインストール時・アンインストール時とも削除しません。
