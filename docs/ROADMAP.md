...
| **DESIGN SPEC APPROVED** | UI-0 documentation and owner decisions are approved. This is not implementation acceptance and does not authorize UI code by itself |
...
- Issue #25 remains open. **UI-1B is next, but NOT STARTED and NOT AUTHORIZED.** The owner's pre-authorization decisions for it are in [UI0_DESIGN_SPEC.md](UI0_DESIGN_SPEC.md) §20, decision 8. No product generation is active. *(Superseded: UI-1B was authorized on 2026-09-25 and merged on 2026-09-26; see the next note.)*

## Position note — 2026-09-26

- **UI-1B (Broadcast Overlay polish) is `ACCEPTED — UNRELEASED`.**
  - The owner authorized it on 2026-09-25 ([record](https://github.com/TullysAC6/ac6-winloss-tracker/issues/25#issuecomment-5832871482)).
  - [PR #48](https://github.com/TullysAC6/ac6-winloss-tracker/pull/48) exact head `499939f2e3e5ccfcfd25972fe8be3037172afe33` passed independent review and real-machine T3 on a 1920×1080 OBS source ([record](https://github.com/TullysAC6/ac6-winloss-tracker/pull/48#issuecomment-5844417204)).
  - It merged as `b64ce73c27b7b5c65fa5203ec90bdb85886b0d3b`, whose tree is identical to the accepted head. [Exact-main CI 36230655908](https://github.com/TullysAC6/ac6-winloss-tracker/actions/runs/36230655908) passed.
  - Public stable v1.2.0 does **not** contain it.
- **What merged:**
  - the Broadcast BEST STREAK toggle `broadcast_show_best_streak` (`preferences.json` v2, default ON, live without restart);
  - a static ordinary streak status with no blink;
  - the 24 px bold text, existing copy and 22 px top-left default, all kept;
  - safer warning handling on narrow sources.
...
[Sequencing](#sequencing--dependency-order-corrected-2026-09-13) below, and that section wins on any
question of *when*.
...
**Cross-cutting (extended 2026-09-13)**
...
3. UI-1A Player Overlay polish - **ACCEPTED — UNRELEASED**, merged in PR #46 (`c2b00dc`); then UI-1B Broadcast Overlay polish - **ACCEPTED — UNRELEASED**, merged in PR #48 (`b64ce73`)
...
7. UI-4 Tray / Launcher modernization ([#26](https://github.com/TullysAC6/ac6-winloss-tracker/issues/26)), last

Prerequisite foundations may be implemented before the visible feature that depends on them.

---

## Sequencing — dependency order, corrected 2026-09-13
...
| **#28-A lands between #15 and UI-2** | #15 provides trustworthy match / observation timestamps. #28-A then defines Season catalog reconciliation and derived assignment. UI-2 can build Settings manual refresh and History Season state once against those settled contracts |
...
| **#14 must carry the later fixture families** | See Phase D. The harness is built once and then extended, so its layout has to accommodate pre-reset, transition, post-reset, missing-Season, multi-season-gap, pre-S and S fixtures rather than being retrofitted for them |
...
| Rank / Rating acquisition | PLANNED | Added for [#28](https://github.com/TullysAC6/ac6-winloss-tracker/issues/28). Event-driven only — **no continuous OCR** |
| Rank / Rating persistence | PLANNED | Conceptual shape in MASTER_REQUIREMENTS §64–§65. Rating observations survive unresolved Season assignment |
...
### Phase 7A.1 — Season Catalog / Assignment Foundation (#28-A)

| Item | Status | Notes |
|---|---|---|
| GitHub Season Catalog / Manifest + fixed-schema validation | PLANNED | Data, never code; bounded HTTPS download and parsing; no user data upload |
| Manual Settings refresh + local cache | PLANNED | Manual-first. Failure keeps the last valid cache and never blocks startup or result recording |
| Retrospective Season assignment | PLANNED | Uses confirmed intervals and saved timestamps; changes derived Season metadata only |
| Multiple-season reconciliation | PLANNED | Fetch all missing definitions. Never infer `season_id + 1` or boundaries from cadence |
| Transition / unresolved handling | PLANNED | Reset gaps remain unresolved / transition; never forced into an adjacent Season |
| Idempotency | PLANNED | Stable `season_id`; repeated refresh/reassignment creates no duplicates and converges to the same state |

Known seed: Season 16 starts `2026-07-24T18:00:00+09:00`; reset starts
`2026-09-25T16:00:00+09:00`; Season 17 start is unknown until reset completion is confirmed.
...
**#16-A / #28-B** is the core growth and seasonal rank/rating analytics data, scheduled after
#15, #28-A and UI-2, then presented by UI-3A; **#16-B** is the advanced analytics (rank-relative depth, matchup and improved-matchup,
...
| Per-season Rank / Rating progression (#28-B) | PLANNED | Uses resolved Season data only and can recompute after reconciliation. Unresolved records are never silently assigned to the current Season. MASTER_REQUIREMENTS §64–§65 |
...
Order within this phase is set by [Sequencing](#sequencing--dependency-order-corrected-2026-09-13)
...
| UI-1B | Broadcast / streaming Overlay polish — stream-safe typography, OBS safe area, scene composition, viewer-distance readability. Primary text keeps the current 24 px bold baseline unless real OBS evidence shows a readability problem (owner decision 2026-09-23, spec §20). Owner pre-authorization decisions of 2026-09-25 (spec §20, decision 8): BEST toggle default ON, existing wording and 22 px placement kept, the BEST toggle as the only new setting, reduced motion deferred. As merged: the Broadcast BEST toggle `broadcast_show_best_streak` in `preferences.json` v2, a static streak status, and safer narrow-source warning handling | **ACCEPTED — UNRELEASED** (PR #48, merged as `b64ce73`) | low |
| Shared visibility settings | 連勝ステータスを表示 and 最高連勝を表示 each become one user preference shared by the Player and Broadcast overlays; the renderers may remain separate ([#25](https://github.com/TullysAC6/ac6-winloss-tracker/issues/25#issuecomment-5843053161), [DECISIONS.md](DECISIONS.md#shared-visibility-settings-across-the-two-overlays)). Open questions: whether BEST returns to the Player Overlay, the shared defaults, and how `player_streak_status_enabled` / `broadcast_show_best_streak` migrate deterministically with one-generation rollback. **Not started; needs separate owner authorization** | BACKLOG | not classified |
