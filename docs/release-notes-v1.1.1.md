...
This project treats a published tag and Release as immutable, so the fix ships as a new version rather than as a correction to v1.1.0. The `v1.1.0` tag and Release are left exactly as published.
...
- Version metadata moved to 1.1.1: `VERSION`, installer `SourceTag`, installer `$version` (which the installer checks against the downloaded source), and the installer and bootstrap User-Agent strings.
